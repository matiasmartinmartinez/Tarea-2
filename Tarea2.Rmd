...
med %>% drop_na() %>% select(Edad_años ,Sexo ) %>% ggplot(aes(Edad_años)) + geom_freqpoly(aes(colour=Sexo),binwidth = 2) + labs(x="Edad en Años",y="Frecuencia absoluta", title="Histograma: distribución de la edad según sexo")
...
```{r}
med %>% select(Prestador_tipo, Importe) %>% ggplot(aes(Prestador_tipo,Importe)) + geom_bar(aes(colour=Prestador_tipo),stat="identity",show.legend = F) + labs(x="Tipo de prestador",y="Monto total en pesos pagado en el año 2016", title="Gráfico de barras: importe total según tipo de prestador")
```
